...
LiDAR_raw_data_file = open("data3.txt", "r")
...
for i in range(5):
    ...
    LiDAR_x, LiDAR_y = LiDAR_raw_data_file.readline().strip().split(' ')

    for j in range(DataPointsPerScan):
        point_distances[j] = float(LiDAR_raw_data_file.readline().strip())
        # x=rcostheta, y=rsintheta
        coords_x.append(
            (float(LiDAR_x) + (point_distances[j] * np.cos(angles[adjust(j + int(DataPointsPerScan/4), DataPointsPerScan)]))))
        coords_y.append(
            (float(LiDAR_y) + (point_distances[j] * np.sin(angles[adjust(j + int(DataPointsPerScan/4), DataPointsPerScan)]))))
